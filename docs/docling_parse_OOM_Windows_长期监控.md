...
> **状态（2026-06-06 起，2026-06-07 大幅更新）**：根因在 IBM 的 docling-parse C++ 库
> （Windows 专属 bug，官方已知、在修但未修好）。DocIngest 侧已落地一套**工程绕过（分批
> 兜底 + 大文件主动分批，已实测有效）**，问题不再阻塞使用。**但根子在上游、没根治**——
> 必须**长期监控 docling-parse 的官方修复**（见 §4 待办 4）。
>
