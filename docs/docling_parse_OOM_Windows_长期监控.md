...
> **状态（2026-06-06 起；✅ 2026-07-04 关案：上游已修复，本机已升级验证，收尾完成）**：
> 根因在 IBM 的 docling-parse C++ 库（Windows 专属 bug）。**docling-parse 7.4.0+（PR #287）
> 已修复**。本机 2026-07-04 升级 docling 2.109.0 + docling-parse 7.5.0 并三项实测全过
> （75 页整篇 SUCCESS / 表格精度无回归 / 519 页整本 SUCCESS 峰值仅 3.1GB），
> 主动分批已停用（`proactive.enabled: false`，用户拍板——新版内存平坦，分批无收益），
> OOM 被动兜底层按待办 6 共识**保留不删**。
> 详见待办 4 的 2026-07-04 条。本档转入**归档状态**：monitoring 任务结束，
> 除非 OOM 复发（届时兜底层会接住并 fail loud），否则无需再定期复查。
>
> **⚠️ 2026-06-07 重要更正**：本文档早先多处写"官方修好后删掉绕过、回原生路径"。
> **经讨论已更正为三层防护架构（见 §4 待办 6）**：① 整本正路 ② 大文件主动分批（控内存，
> **IBM 修好后仍需要**——内存随页数线性增长是架构问题，与崩溃 bug 无关）③ OOM 兜底
> （**保留作长期最后防线，不撤**——它防的是任何解析失败，不止当前这个 bug）。下文个别
> "修好就删"的旧措辞已就地标注更正，但保留原文以存历史推演。
>
> 本文档因此**从"临时交接"转为"长期监控档"**，不要删。给后续维护者 / Agent 用：
> 先读 §0 工作铁律 → §1 现状一图 → **§4 待办 6（三层防护，当前架构）** → §4 待办 4（监控）。
...
- **第二层：内存爆本身（已绕过 ✅，非根治 ⚠️）** —— bug 仍在 docling-parse 里，无法在我们这层
  ...
  （实测 75 页 48 崩 → 0，产物完整）。~~官方修好后应删掉它回原生路径~~ **（2026-06-07 更正：
  OOM 兜底保留作长期最后防线，不撤；详见 §4 待办 6。要长期监控的是 docling-parse 上游修复，
  修好后大文件可回整本正路、上调主动分批阈值，但兜底层留着。）**
...
**已落地代码** 见 §6。（~~官方修好后整体删~~ **2026-06-07 更正：被动兜底保留作最后防线，
不删；详见待办 6**。）
...
~~删掉我们的绕过、回原生路径~~ **（2026-06-07 更正：不是"全删回原生"，而是上调主动分批
阈值让中等文件回正路、保留 OOM 兜底层；见待办 6 + 本条"修好后怎么收尾"）**。这条要**长期
挂着、定期看**。
...
- **修好后怎么收尾（⚠️ 2026-06-07 更正：不是"全删"，是"调阈值 + 保留兜底"）**：
  早先这里写"删掉全部绕过回原生路径"。**已更正**（见待办 6 三层防护）：
  1. 升级 docling / docling-parse 到修复版。
  2. **上调主动分批阈值** `parsing.pdf.oom_batch_fallback.proactive.above_pages` 从 50 到 ~150
     （需先实测"无 bug 整本内存曲线"定准）——中等文件回整本正路，只有巨无霸才主动分批。
  3. **OOM 被动兜底（`_parse_pdf_batched` + 假绿灯 status 检测）保留**，作解析失败的最后防线
     （它防的是任何失败，不止这个 bug）。**不删**。
  4. 重新跑 75 页 WEO 端到端，确认中等文件回原生路径 `status=SUCCESS`、阈值调整无回归。
...
**【2026-06-20 定期检查记录】仍未修复；升 docling 主包治不了（根子仍在 docling-parse）**

- **上游版本**：docling-parse 仍 **6.2.0**（5-28 至今没发新版，根子没动）；docling **主包**倒是
  发了一堆——已到 **2.104.0**（6-19，本机是 2.96.1，主包更新了近 10 个版本）。
- **⚠️ 关键：升 docling 主包不治这个 bug**。6-17 @arkp1 在 **docling 2.99.0** 上实测仍
  `std::bad_alloc`（70 页 PDF、含图含表，34-44 页崩，RapidOCR 部分页空）——印证根因在
  docling-parse 6.2.0 那个没动的 C++ 库，**本机就算把 docling 升到最新 2.104.0 也大概率治不了**。
- **issue #227 仍 closed 未重开**（75 评论，`updated_at` 6-17）。维护者 PeterStaar-IBM
  **6-12 第二次**承诺"未来两周"看（5-21"本周末出"已跳票一次），至今无修复。@simonschoe
  6-08 复盘：用维护者的 `perf/run_scaling.py` 自测 30-40 个文档（含曾崩的私有文件）都不复现，
  回自己主流程（docling 2.96.1）又崩——再次印证"环境/流程差异，维护者难复现"。
- **三层防护维持原样**，`above_pages=50` 不动。
- **下次复查**：7 月初（PeterStaar 第二次"两周"承诺期之后），看 #227 有无回复/重开 + docling-parse
  有无脱离 6.2.0 的新 release。**教训沿用：issue closed ≠ 修好，6-17 仍有新复现报告涌入。**

**【2026-07-04 定期检查记录】⭐ 上游已修复（docling-parse 7.4.0+），本机待升级验证**

- **修复 PR：docling-parse#287**（"feat: reduce total number of results cached"，2026-07-01 合并）。
  根因与官方早前说法吻合：C++ 层把所有页的解析结果**无上限缓存在内存里**，PR 给缓存加了上限
  （维护者 memray 实测：内存占用降到原来的 ~3%，与 16/500 页缓存比例一致）。
  同时 resolves #227 / #286 / docling#3671。
- **Windows 实测确认（两个独立来源，非官方自说自话）**：
  - @MukulLambat（6-25 提交过极干净的复现：573 页 born-digital PDF、OCR 关、Windows 11，
    7.0.0 仍 424 页 bad_alloc）→ **7-01 用 docling-parse 7.4.0 复测：SUCCESS、573/573、
    0 bad_alloc**（docling 2.107.0 + docling-core 2.85.0 + docling-ibm-models 3.13.3）。
  - @simonschoe（就是"2.96.1 仍复现"、和本机症状同款的长期反馈者）7-01 确认
    "works like charm"。
- **版本线**：docling-parse 6.2.0 后直接跳 7.x —— 7.0.0(6-22) 仍有 bug（MukulLambat 实测），
  修复在 **7.4.0(7-01)**+，最新 **7.5.0(7-03)**。
- **⚠️ 本机升级路径（已核对本机依赖元数据）**：docling 2.96.1（docling-slim）钉死
  `docling-parse <7.0.0,>=6.2.0` —— **不能只升 docling-parse**，须 docling 主包一起升到
  支持 7.x 的版本（上游实测确认可行组合：docling 2.107.0 + docling-parse 7.4.0）。
  注意 docling 2.96.1 → 2.107+ 跨了十几个版本，按测试铁律须实测回归（75 页 WEO 整篇 +
  表格精度抽查），不可只看 import 成功。
- **修好后收尾清单（待办 4 原计划，升级验证通过后执行）**：
  1. 升级 docling + docling-parse 到修复版；75 页 WEO 整篇 `SUCCESS + 0 bad_alloc` 才算数。
  2. 实测"无 bug 整本内存曲线"，据此上调 `proactive.above_pages`（50 → ~150）。
  3. OOM 被动兜底 + 假绿灯 status 检测**保留不删**（长期最后防线，待办 6）。
  4. 别忘了待办 7 的修正：图表密集 PDF 拐点远低于 50 页——上游修好后这条阈值优化可能一并失效/重估。

**【2026-07-04 当天完成：本机升级 + 三项实测验证全过 + 收尾落地 ✅ 关案】**

- **升级**：docling 2.96.1→**2.109.0**、docling-parse 6.2.0→**7.5.0**、docling-core
  2.78.0→2.86.0（ibm-models 3.13.0 未变）。DocIngest import 冒烟 OK（api/pipeline/parser 全通）。
- **三项实测（全用监控档既定标尺，WEO PDF，关 vision/ocr，默认 backend）**：

  | 验证项 | 旧版（6.2.0） | 新版（7.5.0） |
  |---|---|---|
  | 75 页整篇 | 48 页 bad_alloc、PARTIAL、139k 字符 | **SUCCESS 75/75、0 错、239k 字符、73s、峰值 2.5GB** |
  | 表格精度（449-456 页标尺） | 碎片行基线 3（pypdfium 灾难线 589） | **碎片行 0**，Nuclear 行数字精确落位——跨 13 版无回归 |
  | 519 页整本 | 必死（偶发 SIGSEGV / 稳定 timeout），分批探针峰值 8GB | **SUCCESS 519/519、0 错、1.49M 字符、864s、峰值 3.1GB** |

- **⭐ 关键新事实（推翻待办 6 的一个假设）**：内存**不再随页数线性增长**——75 页 2.5GB →
  519 页才 3.1GB（旧版同场景 8GB）。上游的缓存上限修复连"整本解析内存 unbounded 增长"这个
  架构行为也一并治了。含义：主动分批的"控内存"必要性大幅下降，整本正路（质量最优、无分批
  边界 title_path 瑕疵）可以放开到实测覆盖的页数。
- **收尾动作（当天全部落地）**：
  1. 主动分批**直接停用**（`proactive.enabled: false`，用户拍板）：内存平坦化后主动分批在
     实测范围内已无收益，所有 PDF 回整本正路（质量最优）；万一失败被动兜底仍接住。
     （中间态曾按实测把 above_pages 调 50→500，同日被"直接停用"取代；数值留档以备重开。）
  2. OOM 被动兜底 + 假绿灯检测 + 主动分批机制**全部保留**（待办 6 共识），config 注释从
     "TEMPORARY WORKAROUND / DELETE when fixed" 改为"长期最后防线"。
  3. 措辞同步：`config/default.yaml`（OOM 段 + azure_di）、`ARCHITECTURE.md`（速查 + 已知边界）、
     `README.md`（azure_di 卖点 ×2）、`PROCESSING_MODES.md`（vision_only 卖点）。
- **随 bug 消亡/新生的课题**：
  - 待办 7 的"图表密集 PDF 拐点 30 页"——bug 时代产物，随修复失效，不再指导阈值。
  - **新课题（未做）**：全局解析锁（`file_concurrency` 只 overlap Vision、解析串行）的历史依据
    是"并发解析放大 bad_alloc"——bug 修了，解锁并行解析的提速空间值得重测（fc 甜点 1.69x@3
    也是 bug 时代数据，作废重测）。动态防御思路：按可用内存 ÷ 单文件峰值（新版实测 ~3GB）cap 并发。

### 待办 5【2026-06-07 ✅ 已实现：大文件主动分批（= 待办6 的第2层）】

> **状态翻转**：本条 2026-06-07 上午一度"决定暂不实现"，当天下午**用户拍板实现**，
> 已落地为"大文件主动分批"。原"暂不实现"的决策推演保留在末尾存档。

**做了什么**：超大 PDF 不再先撞两遍墙才分批 —— `parse()` 在整本 `_do_convert()` **之前**先
探页数，超阈值就直接走 `_parse_pdf_batched`，跳过注定失败的两遍整篇尝试。产物无缝（复用
分批合并，chunk 仍归原文件，实测验证）。这同时是 §4 待办 6 的"第2层主动分批"。

**已落地配置**（`config/default.yaml` → `parsing.pdf.oom_batch_fallback.proactive`）：
```yaml
proactive:
  enabled: true       # 默认开；false → 只跑被动兜底
  above_pages: 50     # >50 页的 PDF 直接分批（贴合实测拐点；IBM 修好后上调到 ~150）
...
**实测铁证（2026-06-07，关 vision）**：
- 60 页（>50）→ 走主动分批（`oom_batch_fallback_used=True`，日志 `re-parsing 60 pages in batches of 50`），产物**1 个 md、所有 chunk original_file=原文件、58 段 pagebreak 连续**✅
- 40 页（<50）→ 走整本正路（无 oom 标记，没被误切）✅

**撞墙拐点实测（2026-06-07，每页数独立子进程整本解析，关 vision/ocr，干净起点）—— 阈值依据**：
  ...
  > "间歇性 bad_alloc"），不是一条精确线。所以 `above_pages` 取 **50**（贴拐点），
  > **IBM 修好后上调到 ~150**（中等文件届时回整本正路，只有巨无霸才分批）。

<details><summary>原"暂不实现"决策推演（2026-06-07 上午，已被下午的实现取代，存档）</summary>

当时理由：① 治标不治本（不让 519 页从失败变成功，根子在上游）；② 动态超时已兜住主要
痛点（519 页 → 1677s 不再被 300s 冤杀）；③ 反过度防御，为省 ~300s 加分叉属锦上添花。
**下午翻转的原因**：用户从"控内存 + 大文件常态"角度重新定位——主动分批不只是省那 300s，
更是 §4 待办 6 三层防护里"IBM 修好后仍需要"的第2层（控内存），值得正式落地。
</details>
...
**第2层用"内置逻辑分批"还是"物理切割"——✅ 2026-06-07 已定：选内置逻辑分批，已落地**
（见待办 5）。取舍对照如下（决策依据，存档）：

| | 物理切割（pymupdf 切成 N 文件） | **内置逻辑分批（docling page_range）← 采用** |
|---|---|---|
| 控内存 | ✅ | ✅ |
...
> **决策理由**：用户要求"切割不影响 chunk、AI 好搜"。内置分批**天然满足**（产物无缝）；
> 物理切割产物是碎片，要额外做"还原成单文件"才能匹配，更复杂且无额外收益（DocIngest 单机
> 串行，物理切的"独立文件/并行"优势用不上）。故采用内置分批，已落地（待办 5）。
> 物理切割方案就此**否决归档**，不再考虑。

### 待办 7【2026-06-20 实测：多进程能否绕 OOM + 提速？—— 否，记录避免重复踩坑】

> 起因：讨论"本地最快形态"时假设"每文件独立子进程 → 进程退出释放 C++ 内存 → OOM 不发生
> + 多核并行提速"。**烧真金白银实测后，这个乐观假设被推翻**。结论存档，免得以后（含 AI）
> 又拍脑袋去做多进程。素材：`test_docs/_concurrency_bench/` 的 7 份 IEA 报告（图表密集，
> 43~238 页），统一 `max_pages=30` + 禁缓存 + force（公平对照，防缓存白嫖假绿灯）。

**两类对照实测数据：**

| 配置 | 成功 | OOM 次数 | 峰值内存 | 备注 |
|---|---|---|---|---|
| 多线程 file_concurrency=1 | 7/7 | 7 | 4.3GB | 基线 626s |
| 多线程 file_concurrency=3 | 7/7 | 7 | 5.9GB | **370s，1.69x（甜点）** |
| 多线程 file_concurrency=6 | 7/7 | 7 | 7.1GB | 397s（倒退，过并发反噬） |
| 多进程 w=3 **复用进程** | **4/7 ❌** | 11 | 13.3GB | 大文件失败 |
| 多进程 w=1 每文件新进程（关vision） | 7/7 | 6 | 2.4GB | — |
| 多进程 w=1 每文件新进程（开vision） | 3/3 | 2 | 3.1GB | 稳 |
| 多进程 w=2 每文件新进程（开vision） | — | — | — | **BrokenProcessPool 崩溃 ❌** |

**三条被实测纠正的认知（都有数据）：**

1. **多进程绕不掉 OOM。** OOM 是 docling-parse 在**单文件、单进程内、30 页就累积触发**的
   （w=1 每文件新进程仍有 6 次）——这是"文件内部累积"，进程隔离只解决"文件之间累积"，
   解决不了文件内部。换进程躲不掉。
2. **多进程并行在本机会内存爆。** 每进程 docling 模型 ~2GB + 开 vision 页图渲染尖峰 +
   `max_tasks_per_child=1` 频繁重启的瞬时叠加 → **w=2 开 vision 直接 BrokenProcessPool**，
   w=3 飙到 13GB。31GB 机器实际撑不住 2 个开 vision 的 docling 进程并行。
3. **多进程唯一真收益：防大文件失败。** 必须 `max_tasks_per_child=1`（每文件全新进程，
   跑完即退）——pool 默认复用进程时内存照样累积，3 份大文件失败；改每文件新进程后 7/7 全成功。
   但这是"防失败"，不是"提速绕 OOM"。

**结论（当前 OOM bug 未修前提下）：多进程在本机不是更快的解法**——并行内存爆、串行不比多
线程快。**多线程 `file_concurrency=3` 的 1.69x 是当前最实际甜点**（fc=6 反而倒退）。真正
的速度黑洞是"单文件内部 OOM bug 逼每个文件走慢 7-9x 的分批兜底"，多进程治不了它，只有等
上游修复或 `max_pages` 裁页减量。**产物质量：多进程产物内容量与多线程基线一致（进程隔离
未损质，Vision 措辞波动属正常非确定性）。**

**⭐ 图表密集 PDF 的 OOM 拐点远低于 §3-C 那张表（重要修正）：** §3-C 的拐点表（"50 页才
崩"）是**纯文本/默认条件**下测的。本轮发现**图表密集型 PDF（IEA 报告）30 页就触发 OOM**
——单页内容密度越高、单页内存越重，拐点越低。**含义：`above_pages=50` 这个主动分批阈值对
图表密集文档偏高**（30 页就该分批却没主动分批，走了"撞墙→被动兜底"的慢路）。这是新的优化
点：阈值应与"单页内容密度"挂钩，不是固定页数。（暂记，不急改——属调度优化、零精度损失。）
...
**2026-06-07 新增（commit 26a7e5d，三层防护 + 周边改进，部分长期保留）：**
- **大文件主动分批**（待办 5/6 第2层）：`config` 的 `parsing.pdf.oom_batch_fallback.proactive`
  段；`docling_parser.py` 的 `parse()` 前置判断 + `_parse_pdf_batched(batch_size=...)` 参数 +
  新增 `_probe_pdf_pages()`。**IBM 修好后保留，只上调阈值**（不随被动兜底删）。
- **动态超时**（独立改进，长期保留）：`config` 的 `parsing.dynamic_timeout` 段；
  `pipeline.py` 的 `_resolve_parse_timeout()` + `_probe_page_count()`，超时按页数缩放。
- **失败不沉默**（独立改进，长期保留）：`api.py` 的 `_surface_failures()` + `ingest()` 加
  `raise_on_failure` 参数（库路径失败必有 warning，可选硬失败）。
- **chunk 瘦身 + visualizer 修复 + score_note**（独立改进，长期保留）：`pipeline.py` 黑名单
  剔 `page_sizes`/`vision_triage`/`oom_batch_fallback_used`，并把 `page_sizes` 接进 index.json；
  `quality_report.py` 加 `score_note` 自解释字段。
> 注意：上面 4 组里只有"主动分批"和 OOM 兜底相关；动态超时 / 失败处理 / chunk 瘦身 /
> score_note 是**独立的好东西，与 docling bug 无关，IBM 修好后全部保留**。
...
**✅ 案子已结（2026-07-04）**：上游 docling-parse 7.4.0+ 修复了根因（缓存无上限累积），
本机已升级 docling 2.109.0 + docling-parse 7.5.0 并三项实测全过（75 页 / 表格精度 /
519 页整本，见待办 4 的 2026-07-04 条）。三层防护架构**保留**：① 整本正路（≤500 页，
上调自 50）② 巨无霸主动分批（>500 页，未实测领域的保护）③ OOM 被动兜底（任何解析失败
的最后防线，长期不删）。监控任务结束，无需再定期复查——若 OOM 复发，兜底层会接住并
fail loud，届时再回来看这份档案。**严守测试原则，别造假绿灯。**
