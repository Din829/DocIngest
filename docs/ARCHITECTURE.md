...
**ground truth 按 sheet 切片（batched + per-page 两条路径都做）**：整本 markdown 当 ground truth 会让每次调用重复背整本（实测 batched 5×192K 字符、per-page 16×~19K token，是 input token 的大头）。`parsing.xlsx.vision.ground_truth_slice`（默认开）让每次调用只拿**它自己的 sheet 段**：batched 在 `_run_one_batch` 内调 `_xlsx_batch_ground_truth_slice`，per-page 在分支准备处经 `_xlsx_per_page_ground_truth`（逐页单页 batch 复用同一 slicer）建 page→slice 字典。归属判定复用 sheet_triage 的锚定区间规则，**任何疑点（未映射邻居 sheet、页不在锚定范围、section/sheet 数不匹配）→ 该次调用回退整本**（切片前的原行为，永不丢内容）。docx 没有 sheet→page 映射 → 检测为不可切 → 自动整本（docx per-page 的成本问题仍在，见 §9.6）。缓存自洽：cache key 含 ground truth hash（`pt_tag`），切片产生的新 ground truth 自动 miss 旧条目。
...
### 9.6 docx 的 Vision 双路径成本问题（待整体重审，勿单点补丁）

2026-06 用 92 页修士论文实测发现的一组**同根**问题，根因都是「docx 嵌入图路径和页图路径在读同一批内容、互相不知道对方」：

1. **per-page ground truth 整本喂**：docx 默认 supplement（`parsing.docx.vision.supplement_only: true`），每页调用背整本 markdown（92 页 × ~102K token ≈ 9.3M input token / 文件）。docx 没有 sheet→page 映射，xlsx 的切片方案（§5.1）搬不过来——页与 markdown 段落的对应关系结构上建不起来（§9.4 Mode B 同根因）。且 `vision.max_pages` / `parsing.docx.max_page_images` 默认 null，无任何闸门。
2. **成本预估失真**：`safety.py` 按「固定单价 × 调用次数」估价，不知道 supplement 调用背着整本小抄——预算审批的数字可能比真实成本低数倍。
3. **双转写冗余**：已 inline 转写的嵌入图被页图 Mode B 再抄一遍（实测 27 行字面重复 + 更大的语义重复面）；同时嵌入图 cap（`max_images_vision`，默认 30）跳过的图又恰好被页图兜住——一边砍预算一边全量跑。cap 本身是「事后截断」形态，理想方向是把嵌入图真实成本算进 Phase 0 预算体检走 warn/strict 审批（检测 + 知情），而非私设张数门槛。

**为什么不单点修**：1 的正解可能是「页图对 docx 的增量价值重审」（嵌入图已覆盖图内容，页图全送的必要性存疑），2 的公式依赖 1 的结论，3 的去重和 cap 都会被 1 的结果改变形态。需要拿真实 docx 样本（含纯文字 / 图文混排 / 扫描页）实测后整体设计。
