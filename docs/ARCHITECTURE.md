...
| **1.3 页图生成** | xlsx/docx/pptx → LibreOffice → PDF → 截图；顺手从中间 PDF 提取导航数据——xlsx 抽 outline 建 sheet→page 映射（Step 1.5）、docx 抽逐页文本层做 per-page ground truth（Step 1.5b，§5.1 切片用） | `_ensure_{excel,docx,pptx}_page_images` |
...
| **1.5 Vision 增强** | 逐页调 Vision（10 层 triage + 并发缓存 + ground truth 切片 + supplement 标题降级，见 §5.1）；按格式分流 supplement/full——xlsx 只补视觉不重抄、PDF/PPT 整页转写（§5.1） | `_enrich_with_vision` |
| **1.6 pre_write hook** | 写盘前最后处理（exiftool、aliases/tags 派生、PII sanitize） | `run_post_parse_hooks(phase="pre_write")` |
| **1.7 Vision dedup** | full 模式页按 `output.vision_keep` 选保留哪半（both/vision/docling，**默认 vision**——按 marker 切分，不靠长度比猜）；supplement 格式（xlsx）和 Mode B（docx）自动保护不参与（§5.4） | `_dedup_vision` |
...
├─ skills/                           Refine prompt 模板（`docingest skills list` 枚举）
│  ├─ refine_default.SKILL.md        允许改写润色
│  ├─ refine_faithful.SKILL.md       逐字保留，只去重和重排版
│  └─ refine_html.SKILL.md           HTML 风格排版输出
├─ src/docingest/
│  ├─ __init__.py                    Public API 导出（ingest / inspect / refine / list_knowledge / get_summary / Provider 类）
│  ├─ api.py                         Facade 主体（ingest / inspect / refine / list_knowledge / get_summary + IngestResult + build_config）
│  ├─ providers.py                   Provider 类（GeminiProvider / OpenAIProvider / AzureOpenAI / Bedrock / VertexAI / DashScope / ...）
│  ├─ cli.py                         typer CLI：run / inspect / refine / doctor / visualize / skills list（直接走 pipeline，不经 facade）
│                                    + 条件挂载 graph 子命令组（lightrag 装了才出现）
│  ├─ mcp_server.py                  MCP server（薄壳转调 api.py）：3 个核心工具（inspect / run / refine）
│                                    + 4 个 graph 工具（build_graph / query_graph / graph_status / enrich_chunks，条件注册）
...
│  │  ├─ xlsx_connector.py           post_parse：xlsx AutoShape 连接线直读（画面遷移図/フロー図的箭头关系）
│  │  ├─ file_metadata.py            pre_write：Docling origin 提升 + exiftool
│  │  ├─ derive_aliases.py           pre_write：metadata["aliases"] 派生（文件名/标题别名，给 Obsidian/RAG）
│  │  ├─ derive_tags.py              pre_write：metadata["tags"] 派生 stage 1（format/lang 标签；stage 2 见 output/tags_enrichment）
...
│  │  └─ token_tracker.py            全局用量账本（API 返回值原样累计：prompt/completion
│  │                                 + reasoning/cached/audio-seconds 明细桶 + provider 自报
│  │                                 total 作对账锚点；ASR/embedding 也进账）
│  ├─ utils/
│  │  ├─ binary_finder.py            跨平台定位 soffice / exiftool / ffmpeg / yt-dlp
│  │  ├─ bundled_binaries.py         打包 exe / imageio-ffmpeg 自带二进制的路径注入
│  │  ├─ encryption.py               加密文件检测（parse 失败后定性"密码保护"→ error_type=encrypted）
│  │  ├─ library.py                  知识库管理（list/summary 已建库，支撑 api.list_knowledge/get_summary 和 GUI）
│  │  ├─ resources.py                资源根定位（dev 与 PyInstaller-frozen exe 双形态）
│  │  ├─ timeout.py                  run_with_timeout（Vision 调用墙钟保护）
│  │  ├─ xlsx_sheet_map.py           sheet→PDF 页映射（LibreOffice outline 书签，sheet_triage / GT 切片用）
...
│  │  ├─ tags_enrichment.py          tags 派生 stage 2（knowledge_map 建好后追加主题标签到 frontmatter）
...
| Vision 10 层 triage | `pipeline.py::_should_skip_vision` |
| 文件级 overlap + 全局 Vision 信号量 | `pipeline.py` `run_pipeline`（`performance.file_concurrency`，§9.1） |
...
- 公开名字（`ingest` / `inspect` / `refine` / `list_knowledge` / `get_summary` / `IngestResult` / `build_config` / Provider 类）的**函数签名 + 返回值语义**在同一 minor 版本内不破坏。
...
| **post_parse** | 解析完成、Vision **之前** | 无 | 注入结构化数据给 Vision 做 ground truth | PPTX chart 直读 (`hooks/pptx_chart.py`)、xlsx 连接线直读 (`hooks/xlsx_connector.py`) |
| **pre_write** | Vision 之后、写盘**之前** | 无 | 加元数据 / 改 markdown | file_metadata、derive_aliases、derive_tags、sanitize |
...
3. 让 `auto` 策略选中你的 chunker：config 加 `chunking.auto.format_strategies.<ext>: "my_strategy"`，并在 `AutoChunker._select_strategy()`（`chunkers/__init__.py`，grep 函数名定位）里加 dispatch

---

## 5. 关键机制详解

### 5.1 Vision 增强与 10 层 triage
...
parsing:
  vision:
    enabled: true
    max_pages: null         # 全局 cap，null（默认）= 无限制；成本闸门交给 Phase 0 safety
    image_dpi: 180
    triage:
      enabled: true         # 预筛选：纯文本页跳过 Vision，省 30-60% API
      min_text_length: 50
      max_replacement_ratio: 0.05
      table_line_threshold: 30
      max_mixed_script_fragments: 8
...
#### 10 层检测（全通过才跳过 Vision）

`pipeline.py::_should_skip_vision`（权威列表看函数 docstring）：

1. 无 `<!-- image -->` 标记
2. 无 picture 元素（Docling figures / fitz 探测的嵌入图）
3. 无 PPTX chart 等 hook 注入的结构化数据
4. 文本长度 ≥ `min_text_length`
5. 无 `glyph<` / `glyph&lt;` CID 失败标记（含 CJK 邻接 `&lt;` 实体变体）
6. U+FFFD 占比 ≤ `max_replacement_ratio`
7. Markdown 表格行数 < `table_line_threshold`
8. 无混合脚本碎片（CJK 之间夹短 ASCII，OCR 误识特征）
9. 脚本与声明语言一致（catches CMap 失败产出合法但错误 Unicode——比如日语 PDF 出现大量孟加拉语字符）
10. 无拉丁字母替换密码式乱码（CMap 坏掉但输出全是合法 ASCII，靠元音占比异常检出；仅拉丁文档）

另有 **furniture 豁免**（`triage.furniture`，默认关、仅 PDF）：页上唯一的图是跨页重复的 logo/页眉脚时豁免"有图必送"两关，其余层照常可否决。xlsx 还有 **sheet 级 triage**（`parsing.xlsx.vision.sheet_triage`，默认开）：零 drawing/ole 元素的 sheet 整段跳过（锚定页区间判定，疑点不跳）。

**设计动机**：false negative（多送 Vision）代价很小；false positive（该送没送）丢信息代价大。所以门槛故意偏严。
...
**ground truth 切片（batched + per-page 两条路径、xlsx + docx 两种来源）**：整本 markdown 当 ground truth 会让每次调用重复背整本（实测 batched 5×192K 字符、per-page xlsx 16×~19K token、docx 92×~102K token≈9.4M，是 input token 的大头）。每次调用只拿**它自己的那份**：

- **xlsx**（`parsing.xlsx.vision.ground_truth_slice`，默认开）：按 sheet 段切。batched 在 `_run_one_batch` 内调 `_xlsx_batch_ground_truth_slice`，per-page 经 `_xlsx_per_page_ground_truth`（逐页单页 batch 复用同一 slicer）。归属复用 sheet_triage 锚定区间规则，任何疑点（未映射邻居 sheet、页不在锚定范围、section/sheet 数不匹配）→ 该次调用回退整本。实测 28-sheet 工作簿 input 270K→34K（-87%）。
- **docx**（`parsing.docx.vision.ground_truth_slice`，默认开）：docx 没有 sheet 映射，但生成页图的 LibreOffice 渲染**顺手产出了带文本层的 PDF**——分页是现成的。Step 1.5b（`_generate_page_images_via_libreoffice`）用 pymupdf 逐页抽文本存 `metadata["docx_page_texts"]`（已入 chunk metadata 黑名单 §5.14）；`_docx_per_page_ground_truth` 给每页拼小抄 = **该页 PDF 文本 + 该页已转写嵌入图的转写块**（嵌入图按"marker 上方/下方最近正文行在 PDF 唯一命中页"锚定，实测 65/71；锚不上的图不进小抄——最坏被该页重抄，内容不丢）。实测 92 页论文 input 9.4M→264K（-97%），4 页误判重抄转正确 no-supplement、12 页漏补找回。扫描页文本层为空 → 空小抄正确触发"整页转写"逃生。

接线为检测式不分格式：xlsx 没有 `docx_page_texts`、docx 没有 sheet 映射，两个 slicer 恰好只有一个（或都不）武装。缓存自洽：cache key 含 ground truth hash（`pt_tag`），切片产生的新 ground truth 自动 miss 旧条目。

**supplement / 嵌入图转写的标题降级**（`parsing.vision.supplement_demote_headings`，默认开）：Vision 转写会给图表标题 / 轴例写 `#`/`###`。full 模式（PDF/PPT）的标题**是**页面真实结构（Vision 在重建 Docling 拍碎的版面）必须保留；但 supplement 块和嵌入图转写里它们是**图内标签**，会冒充文档章节——实测一篇正文章节全用粗体（无 Word 标题样式）的论文，图表名成了全文档唯一标题，骗过 auto 结构评分、118/325 个 chunk 的 title_path 挂着某张图表名。注入前 `_demote_headings` 把标题行降为粗体：文字全保留（chunk 内容 / grep / 召回不变），只剥结构权。实测降级后 auto 正确回落 recursive（空 title_path 优于错的），带真标题样式的 docx 不受影响（title_path 正常生成）。

**Vision 失败页可审计**：per-page 调用彻底失败（超时 / API 错误穿过重试）的页，在该页 section 末尾（Mode B 则文末）写 `<!-- vision-failed page=N -->` marker——产物自己记录自己的缺口，和 `[?]`/`[unreadable]` 同一哲学。`quality_report.py` 扫描该 marker 输出 `vision_failed_pages`（不计入 quality_score——那是"源不可读"维度，这是"调用没跑成"维度）。

#### 并发 + 缓存

- `ThreadPoolExecutor` 并发调用，worker 数读 `performance.parallel_files`（页内并发）；文件级 overlap 是独立旋钮 `performance.file_concurrency`（默认 1=串行），全局 in-flight 上限 `performance.vision_global_concurrency` 防多文件并发打爆 rate limit（见 §9.1）
...
per-format 页数 cap（`parsing.<fmt>.max_page_images`）默认 **null（不限）**——要单独限某格式才设 int，超了 → 只处理前 N 页 + warning。单图超过 `max_image_pixels` (默认 4M) 自动降采样。

LibreOffice 没装 → 所有 office 格式降级为纯文本模式，静默跳过（不报错）。二进制发现走 `utils/binary_finder.py`。

### 5.4 去重：源头 supplement（xlsx）+ vision_keep 选半（PDF/PPT full 模式）

**消除 Docling↔Vision 重复的两条线，按格式分工：**

- **xlsx → 源头 supplement（见 §5.1）**：Vision 只补视觉、不重抄表格，重复在生成时就不产生（无损：openpyxl 那份正文始终保留）。
- **PDF / PPT（full 模式整页转写）→ `output.vision_keep` 选半**：full 模式下一页会有两份内容（Docling 抽取 + `<!-- vision-enriched -->` 后的 Vision 整页转写）。`vision_keep` 决定保留哪半，**按 marker 切分定位，绝不按长度比猜**（旧 `output.dedup` 长度比方案判不了"内容是否覆盖"，已废弃替换）：
  - `vision`（**默认**）—— 删 Docling 半、留 Vision 半。纯文本 PDF/PPT 上 Vision 通常是超集（能拿到手写签名、填空、Docling 漏的结构字段）
  - `both` —— 两半都留（零内容损失，= 旧默认行为）
  - `docling` —— 留 Docling 半

**安全边界（自动保护，配置无法误伤）**：supplement 模式格式（xlsx——Vision 那半只有视觉补充，表格正文在 Docling/openpyxl 半边）和 Mode B 的 docx（Vision 追加在文末、与 Docling 文本不构成页对齐的超集对）**强制保持 both**，`vision_keep` 对它们不生效。

**何时改回 `both`**：你的下游想同时比对两个视角，或你不信任 Vision 转写质量想保底。代价是 `sources/*.md` 大 30-100%；chunks 里重复内容会分到相邻 chunk，对 RAG 召回反而无害。
...
- 页数靠 `pipeline.py::_probe_page_count`（PyMuPDF xref，O(1)）探，**仅 PDF**；非 PDF 或探测失败 → 回退固定 `parsing.timeout_sec`（默认 600）。探测失败绝不阻塞解析。
...
- 剔除的典型：`element_boxes` / `page_sizes`（几何数据，**改存 index.json**，visualizer 从那读）、`docling_origin` / `binary_hash`（血缘走 `lineage.original_input`）、`xlsx_sheet_page_map`、`docx_page_texts`（整本 PDF 文本层 ~100KB，只服务 §5.1 的 per-page 切片）、`vision_triage` / `oom_batch_fallback_used`（运行级诊断，进 run_log 不进 chunk）等。
...
### 5.15 派生 metadata：aliases / tags / created（frontmatter 白名单制）

给 Obsidian / RAG 消费者的轻量元数据，全部从已抽取的数据合成、零额外 LLM 调用，开关在 `output.derived_metadata.*`：

- **aliases**（`hooks/derive_aliases.py`，pre_write）：文件名 / 标题派生的别名短列表，提高 grep / 向量检索命中。
- **tags 两段式**：stage 1（`hooks/derive_tags.py`，pre_write）写文件本地的 `format/<f>`、`lang/<l>`；stage 2（`output/tags_enrichment.py`，knowledge_map 建好**之后**回写）追加主题标签——所以主题标签依赖 knowledge_map 开着。
- **created / created_source**：内嵌日期优先，回退文件系统 mtime。

frontmatter 输出受 `output.markdown.frontmatter_fields` **白名单**控制：字段要同时满足"在白名单里 + hook/parser 真写进了 metadata"才会出现——内部状态不会泄漏进 frontmatter。hook 加新字段时两边都要登记。

### 5.16 其它小机制速查

- **加密文件检测**（`utils/encryption.py`）：只在 parse 失败后运行（正常文件零开销），识别密码保护 → `error_type="encrypted"`，把"document is not valid"这种谜语报错变成可操作的原因。
...
### 9.1 文件级 overlap（已实装，默认关）—— 解析永远串行

**历史**：本节曾论证"文件级并行刻意不实装"（rate-limit 乘数、共享状态加锁、pymupdf 非线程安全）。2026-06（commit `98d1780`）按当时设想的安全形态实装了——**不是全量并行，是受控 overlap**：

- **`performance.file_concurrency`（默认 1）**：1 = 完全串行，行为与从前一字不差。2-3 = 文件 B 的解析与文件 A 的 Vision I/O 等待重叠。
- **解析永不并发**：全局 parse lock 串行化所有解析（probe 实测：Docling 模型每 parser 实例 ~1.9GB RSS，并发解析还会放大 docling-parse Windows std::bad_alloc）。收益全部来自"解析与 Vision 等待重叠"——Vision 等待占大头的 batch 才提速。
- **`performance.vision_global_concurrency`（默认 null=跟随 parallel_files）**：跨文件 Vision in-flight 总量上限，正是原"触发重估"段设想的全局 LLM semaphore——没有它，`file_concurrency × parallel_files` 个并发调用会打爆 rate limit。
- **输出顺序不变**：聚合在主线程按输入序进行，index.json / chunks.jsonl 与串行跑完全一致。进度事件会交错——一次只显示一个文件的 UI 请保持 `file_concurrency: 1`。

CLI 入口 `--parallel-files`。当前默认仍是串行（=旧行为），所以本节从"技术债"降级为"默认保守的可选加速"。
...
### 9.6 docx 的 Vision 双路径成本问题【2026-06-12 ✅ 已整体解决，留残留观察项】

92 页修士论文实测发现的一组同根问题（嵌入图路径和页图路径读同一批内容、互相不知道对方），当天以三个独立 commit 整体落地（`cba717b` / `d832ec7` / `3643434`），机制细节见 §5.1：

1. **per-page ground truth 整本喂（9.4M token/文件）** → ✅ docx 经 LibreOffice PDF 文本层逐页切片——"docx 切不了"是伪命题，分页 LibreOffice 渲染页图时早就算好了。实测 -97%。
2. **成本预估失真** → ✅ 切片后每次调用回到"页图+小抄 ~3K"量级，`safety.est_tokens_per_vision_call.input` 据实重校准 40000→8000（实测预估 $2.12 vs 实付 ~$1.3，合格上界）。
3. **双转写冗余 + cap 事后截断** → ✅ 聚焦小抄让页图正确判"已覆盖"，重抄在源头消失；`max_images_vision` 30→null 退役（与 `max_page_images` 同一反截断立场），嵌入图全读全计价、由 Phase 0 strict 门审批。

**残留观察项**：① 锚定不上的图（实测 6/71）被所在页重抄——产物体积 ~1% 的冗余行，内容不丢；锚定命中率有提升空间（已加图题后向锚 63→65）。② 标题降级后无标题样式的 docx 回落 recursive、title_path 为空——长远可做"粗体短行 + 章节编号 → 标题"的结构提升（数据飞轮方向，见 COMPETITIVE_POSITIONING §3.5）。
