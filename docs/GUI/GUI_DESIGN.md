...
> **实现状态（已落地）**：本设计已落地为 `src/docingest/gui/`（pywebview + 手写
> 前端，三层解耦）。落地范围**超出本文最初的"第一版只做主链"计划**——
> inspect / ingest / **refine / 知识图谱构建** / doctor / 设置 / 产物 Markdown
> 预览均已实现，前端 9 屏（每页带返回按钮）。本文保留为**设计依据 + 踩坑记录**
> （弃 Gradio 的理由、二进制打包的 `0xC0000142` 坑、三层解耦铁律），仍然准确；
> 下文个别"第一版不做 / 后续再接"的措辞是旧计划，已就地标注实际状态。
...
**后端在同一 Python 进程内直调 `docingest.api`，不要 HTTP 层。** DocIngest 后端
几乎不用改——`inspect/ingest/refine` + `on_progress` 进度回调 + safety 成本
预检都现成。唯一要补的是「每次处理用独立输出目录」，避免多次运行互相覆盖。

> 注：pywebview 下 JS 界面**不能直接 import Python**，要经 `js_api` 桥（JS 调
> `window.pywebview.api.x()`，Python 用 `evaluate_js` 推进度）。所以「直调 api.py」
> 指的是**桥类/适配层（Python 侧）直调**，不是 JS 直调。详见 [BACKEND_API.md](BACKEND_API.md)。
...
|---|---|---|
| GUI→本地 HTTP API→核心 | Python 侧（桥/适配层）直调 `api.py`，JS 经 js_api 桥 | 核心轻量，不需要进程隔离 / HTTP |
...
| `refine(files, skill=)` | api.py | 产可读版（**已实现**：`start_refine`，后台线程 + 成本门 + 完成/拦截/错误三事件） | 单状态"整形中…" |
...
> **已有可视化设计稿**：[GUI_DRAFT.pen](GUI_DRAFT.pen)（用 Pencil 打开）——主流程 11 屏，
> 日本咨询风的纯白极简（白底 + 墨黑 + 衬线日文标题 Noto Serif JP）。涵盖
> 空态/已选/处理中/完了 + 设置三屏（环境检查·AI模型·成本上限）+ **知识图谱构建**
> （未构建：构建入口+成本提示；构建中：用语→关系→社区检测四阶段进度）+ 成本确认/
> 整形方式两弹窗。处理中屏含**「文件解析失败」态**（红 ✗，唯一的彩色留给真需警觉
> 的事）；**完了屏右侧已有产物 Markdown 预览区**（文件选择器+渲染内容，预览产物不是
> 输入）。
>
> **知识图谱「查询」页已移存档**（`.pen` 里 `[存档·未来web检索agent参考]`，y=1600）——
> graph 检索/问答归未来独立 web 检索 agent，**不在本地 exe 主流程**；主流程只到
> graph「构建」（构建是「造数据」的延伸，归 exe）。设计稿是落地要还原的「北极星」。
>
> **落地的设计原则**（设计稿已贯彻，落地时遵守）：① **cost 默认藏**——主界面不提
> 成本，点不起眼的入口或超预算时才弹；② **说人话**——`strict`/`fallback`/graph
> 的 `naive/hybrid` 等内部术语翻译成人话或藏进默认；③ **反 AI 味**——不用彩色软底
> 胶囊、不框中框、删冗余小灰字；④ **参数分层**——高频就地调、低频进设置页、专业项
> 藏默认。详见 `参考设计思想/frontend-design-pro.md`。

只定设计/信息架构；具体 UI 组件、控件库、文案语言见上方设计稿。

### 交付形态：本地工具，零部署

**界面层：pywebview + 手写 HTML/CSS（不是 Gradio）。** 后端仍是纯 Python
（本地起一个 FastAPI/轻服务包 `api.py`），界面用 pywebview 起一个原生窗口
加载手写网页。服务起在用户本机（`localhost`），数据不出本机、不联网、不配域名。
交付时用 PyInstaller 打包成 exe：员工双击 → 自动起本地服务 + 弹窗口，体验像桌面
软件，且**连 Python 都不用装**。pywebview 用系统自带 WebView（Win 的 Edge
WebView2），不捆 Chromium，体积小。这正好解决「员工不碰命令行」的诉求。

> **为什么不用 Gradio（已实测）**：最初定的是 Gradio（和 MinerU 一致，最省事）。
> 但实测发现 Gradio 只能套上配色/字体（约六成还原度），自定义报价表、弹层、
> 精确间距这些**做不出设计稿的精致度**——Gradio 的 DOM 是它自己生成的，CSS 只能
> 贴皮。要 100% 还原 [GUI_DRAFT.pen](GUI_DRAFT.pen) 的设计，只能手写前端。
> pywebview 既满足「双击 exe / 数据不出本机」，又能像素级还原，是当前结论。
> （Tauri/Electron+React 也能 100% 还原，但引入 React+Rust/Chromium，违背本项目
> 纯 Python 的克制路线，不选。）
...
实测铁证（`tests/bundle_deps_probe.py` 本机真跑 + 真 PyInstaller 打包跑）：
- `SOFFICE_PATH` 指向自带二进制 → `find_binary` 命中；指向坏路径 → 返回 None 不静默回退。✅
- LibreOffice headless 真转换 PPT→PDF 成功（本机 + 打包态都出 39954 字节 PDF）。✅
- ffprobe 指向不存在 → inspect 正常返回、`duration_sec` 干净缺失、不崩。✅
- 注入逻辑在真打包态（含模拟 + 真 `_MEIPASS`）从 exe 内部找到捆绑二进制并注入。✅

各二进制怎么带：
- **ffmpeg**：用 `imageio-ffmpeg`（pip 包，wheel 自带跨平台 ffmpeg；实测 Windows
  wheel ~31MB），启动时 `os.environ["FFMPEG_PATH"] = imageio_ffmpeg.get_ffmpeg_exe()`。
  已落地在 `bundled_binaries.ensure_bundled_binaries()`（启动时注入）。
...
**打包态已实测通过（含一个真打包才暴露的坑 + 修复）**：真 PyInstaller 打包后，
LibreOffice 从 `_MEIPASS` 被定位、headless 真转换出 PDF（39954 字节）。但路上踩
了一个坑：

> **PyInstaller 的 SetDllDirectory 污染子进程（致 0xC0000142）**。PyInstaller 的
> bootloader 调 `SetDllDirectory` 让自带 DLL 优先加载，**子进程会继承**这个进程级
> 设置 → 打包进去的 soffice 加载到 app 的不兼容 DLL，崩 `0xC0000142`、不出 PDF。
> 这**不是环境变量**（试遍各种 env 清理都没用），是 Win32 进程状态。
> **修复**：起 soffice 前 `SetDllDirectory(None)` 恢复标准 DLL 加载顺序，起完恢复。
> 已落地在 `binary_finder.run_soffice_convert()`（Windows-frozen 才生效），打包态
> 实测从崩溃转为出 PDF。soffice 还需一个可写的 `-env:UserInstallation` 临时 profile
> （helper 一并处理）。三处 soffice 调用（PPT/xls/Excel）统一走这个 helper。

> 这个坑光看代码 / 文档发现不了，是真打包跑出来的——记在此，避免以后重踩。

**前端 pywebview 多一个系统依赖：WebView2（Windows 渲染引擎）**。和上面那两个
不同——它不是 DocIngest 调的外部二进制，是 pywebview 渲染手写 HTML 用的系统组件
（弃用 Gradio 改 pywebview 带来的，换前端框架不影响后端那套捆绑/打包工作）。
- Win10/11 基本自带，多数机器开箱即用；极老机器可能没有。
- 微软有静默安装包（Evergreen Bootstrapper，~2MB），可塞进 exe 安装流程兜底。
- 比 LibreOffice 轻得多，但**打包验证时要确认**：目标机器无 WebView2 时窗口能不能
  起、有没有友好提示。这条**仍未实测**（GUI 本身已实现并在有 WebView2 的机器上跑通；
  缺 WebView2 的兜底行为留待真打包分发时验）。
...
```
① 界面层  (gui_app.py：pywebview + 手写 HTML/CSS；曾评估 Gradio 已弃)  ← 想换就换这层
      只做：收集控件值、显示结果。不写业务逻辑。
② 适配层  (gui_logic.py：把界面输入翻译成 api 调用，把进度/结果整理成易显示的数据)
③ 核心层  (docingest.api 的 inspect / ingest / refine)       ← 永远不动
```

**解耦铁律：适配层的函数签名里不出现任何界面框架的类型。**
- 进度用普通回调 `on_progress(事件dict)`，不用任何框架专属的 streaming 对象。
- 返回用 dict / dataclass，不用界面框架专属的更新对象。
- 适配层对「前端是什么」完全无知 —— 这才是真解耦。

这样 pywebview 版和未来任何前端（PyQt / Web 等）**调同一个 `gui_logic.py`**，
换前端 = 只重写界面层，业务逻辑零改动。

> 反例（会绑死，禁止）：把 `api.ingest()` 直接写进界面框架的按钮回调、进度直接
> 用框架专属对象更新。逻辑和界面框架焊死，换前端就得重写。
...
- **进度有两层：文件级 + 文件内 Vision 页级**（2026-06-08 新增页级）。`on_progress` 发两种事件：
  - `kind="file_done"`：文件完成，`current/total`（第几个文件 / 共几个）+ 文件名 + chunk 数。
  - `kind="file_progress"`：文件**处理中**的子进度——`phase="vision"` 时带 `sub_current/sub_total`
    （Vision 第几页 / 共几页），`phase="parse"`（`sub_total=0`）表示"正在解析"忙碌态（解析是
    Docling 黑盒，给不了百分比）。**GUI 可以做 Vision 子进度条了**——这是因为单文件内 Vision 是最长
    静默段（实测 1.9MB PDF 静默 76s），逐页报把它点亮。老消费者只认 `file_done` 自动忽略新事件。
...
- ~~graph 子命令上 GUI —— opt-in 进阶场景，第一版只做 inspect→ingest 主链。~~
  **（已变更：graph「构建」已上 GUI ——`start_build_graph` + 空态/构建中两屏。
  graph「查询/问答」仍不做，归未来独立 web 检索 agent，见上文存档说明。）**
