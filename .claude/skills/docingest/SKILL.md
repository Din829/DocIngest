...
|---|---|---|---|
| `run` | Process docs → Markdown + chunks + index | Main command. Run after `inspect` for large/unknown inputs; directly for small trusted ones. Re-running the same `-o` is cheap (incremental cache). | `-o/--output` (REQUIRED for multi-input); `--purpose markdown\|rag\|agentic\|full`; `--outputs md,chunks,index,...`; `--no-chunks`; `--engine docling\|vision_only\|azure_di` (vision_only = skip docling on PDF/image, OOM-immune; azure_di = cloud parse, needs [azure]); `--strategy auto\|heading\|recursive\|slide\|sheet\|timestamp\|whole`; `--max-pages N` (parse only first N pages of PDF/PPTX/DOCX); `--parallel N`; `--parallel-files N` (Vision/ASR workers); `--force`; `-y/--yes`; `--json`; `--verbose/-v`; `-c/--config` |
