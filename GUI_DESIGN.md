...
> **已有可视化设计稿**：[GUI_DRAFT.pen](GUI_DRAFT.pen)（用 Pencil 打开）——12 屏完整
> 设计，日本咨询风的纯白极简（白底 + 墨黑 + 衬线日文标题 Noto Serif JP）。涵盖
> 空态/已选/处理中/完了 + 设置三屏（环境检查·AI模型·成本上限）+ 知识图谱（查询）+
> 成本确认/整形方式两弹窗，外加对照后端能力补的 3 处：**知识图谱未构建（构建入口
> + 成本提示）**、**知识图谱构建中（用语→关系→社区检测四阶段进度）**、**处理中屏的
> 「文件解析失败」态**（红 ✗，唯一的彩色留给真需警觉的事）。**完了屏右侧已有产物
> Markdown 预览区**（文件选择器 + 渲染内容，预览的是产物不是输入）。设计稿是落地
> 要还原的「北极星」。
>
