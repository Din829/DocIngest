...
└─────────────────────────────────────────────────────────────┘
  ...
  ▼
┌─────────────────────────────────────────────────────────────┐
│ Phase 1.2: Excel ノイズ除去 (全 xlsx/xls/csv 統一パス)        │
│                                                               │
│ データ型 Excel → 変化なし、レイアウト型 → 大幅ノイズ除去        │
│ ├── 行内重複排除 (結合セル → N 個同一値 → 1 個に折畳)          │
│ ├── 行間重複排除 (rowspan 展開 → 連続同一行 → 折畳)            │
│ ├── 疎セル除去 (空 50% 超 → 空セル削除)                        │
│ └── 埋込画像抽出 (xlsx zip → xl/media/ → assets/)              │
│ AI コストゼロ。設定駆動、各ステップ個別 ON/OFF 可能。           │
└─────────────────────────────────────────────────────────────┘
  │
  ▼
┌─────────────────────────────────────────────────────────────┐
│ Phase 1.3: Excel ページ画像生成 (任意、LibreOffice 必要)      │
│                                                               │
│ Excel には「ページ」概念なし                                    │
│ → LibreOffice headless でPDF化 → ページ単位スクリーンショット   │
│ ├── max_page_images 上限 (既定 10、超過 → テキストのみ+警告)   │
│ ├── max_image_pixels 上限 (既定 400万px、超過 → 自動縮小)      │
│ └── LibreOffice 未導入 → 静音スキップ（テキストモードで続行）   │
│ 生成画像 → Phase 1.5 Vision に供給                             │
└─────────────────────────────────────────────────────────────┘
  │
  ▼
Phase 1.5: Vision 増強 (既存)
  │
  ▼
┌─────────────────────────────────────────────────────────────┐
│ Phase 2: 構造化出力                                           │
...
(任意) `docingest refine` で追加生成:
  ✅ readable/*.md             → 人間可読版 (Mermaid 図、整形テーブル、ノイズゼロ)
```
...
| XLSX/CSV (データ型) | 行テキスト変換 + Phase 1.2 ノイズ除去 (変化小) | Phase 1.3 が画像生成可能 | `xlsx.row_to_text`, `xlsx.denoising.*` |
| XLSX (レイアウト型/仕様書) | 行テキスト変換 + Phase 1.2 ノイズ除去 (大幅削減) | Phase 1.3 → Vision で補完 | `xlsx.denoising.*`, LibreOffice |
| HTML | メインコンテンツ抽出 | 不要 | — |
...
│   ├── q3-proposal.md
│   └── financial-data.md
├── readable/                        ← 人間可読版 (docingest refine で生成、任意)
│   └── annual-report-2025.md
├── assets/                          ← 抽出画像 + Excel 埋込画像
│   ├── annual-report-2025-p12-chart.png
│   ├── 仕様書-image1.png            ← xlsx zip から抽出
│   └── 仕様書-page-001.png          ← LibreOffice 経由のスクリーンショット
├── index.json                       ← ファイル目録
...
├── skills/                      # SKILL テンプレート (refine 用、カスタマイズ可能)
│   └── refine_default.SKILL.md  # デフォルト精修プロンプト
├── src/docingest/
│   ├── cli.py                   # CLI: run + refine サブコマンド
│   ├── config.py                # 設定ロード (default < project < CLI args)
│   ├── pipeline.py              # メイン Pipeline (Phase 1-4) + Excel ノイズ除去
│   ├── refine.py                # AI Refine (独立、sources/ → readable/)
│   ├── parsers/                 # Phase 1: 解析 (可插拔)
│   │   ├── base.py              # ParseResult + PageData + PAGEBREAK_MARKER
│   │   ├── docling_parser.py    # Docling アダプタ + セクション名注入 + xlsx 画像抽出
...
メインパイプラインとは独立した後処理コマンド。`sources/*.md` を入力し、LLM で整形して `readable/*.md` に出力する。

**目的**: 人間が Obsidian / GitHub / Wiki 上で快適に閲覧できる Markdown を生成する。

**設計原則**:
- 情報損失ゼロ — フォーマット整理とノイズ除去のみ、事実内容は一切削除しない
- 原本不変 — `sources/` のファイルは一切変更しない
- 任意実行 — 全ファイルに適用する必要なし（仕様書等、人が読む必要があるものだけ）
- SKILL カスタマイズ — プロンプトテンプレート（`.SKILL.md`）を差し替え可能

### 13.2 データフロー

```
sources/画面設計書.md (Docling + Vision の生出力)
  ↓
AI Refine (LLM 1回呼出)
  ├── Docling テキスト + Vision テキストの重複を統合
  ├── Excel 公式残留 (=MAX(), =ROW() 等) を削除
  ├── HTML コメント (<!-- image -->, <!-- vision-enriched -->) を削除
  ├── テーブルのフォーマット整理
  ├── メタデータヘッダーの構造化
  └── フローチャート記述 → Mermaid 図に変換（該当する場合のみ）
  ↓
readable/画面設計書.md (人間可読版)
```

### 13.3 SKILL ファイル

`skills/` ディレクトリ内の `.SKILL.md` ファイルが LLM の system prompt として使用される。

```
skills/
├── refine_default.SKILL.md        ← 汎用精修（デフォルト）
├── refine_shiyousho.SKILL.md      ← 仕様書専用（ユーザー作成）
├── refine_flowchart.SKILL.md      ← フローチャート重視（ユーザー作成）
└── refine_db_design.SKILL.md      ← DB設計書用（ユーザー作成）
```

CLI で `--skill` オプションで指定:
```bash
docingest refine sources/業務フロー図.md --skill refine_flowchart
```
...
```yaml
refine:
  output_dir: "readable"          # 出力先 (knowledge/ 内のサブディレクトリ)
  model: "chunking_assist"        # models.chunking_assist の設定を再利用
  max_input_tokens: 8000          # 上限超過 → スキップ + 警告
  max_output_tokens: 8000         # LLM 応答上限
  default_skill: "refine_default" # デフォルト SKILL 名
  skills_dir: "skills"            # SKILL ファイル検索ディレクトリ
```

### 13.5 実測結果

宮城県 画面レイアウト定義書（Excel 仕様書）での実測:

| 指標 | Docling 生出力 | Phase 1.2 ノイズ除去後 | AI Refine 後 |
|------|--------------|---------------------|-------------|
| 文字数 | 126,466 | 1,913 | 2,588 |
| トークン数 | 52,419 | 2,341 | 1,845 |
| Excel 公式 | 大量残留 | 残留 | **ゼロ** |
| HTML コメント | — | あり | **ゼロ** |
| Mermaid 図 | — | — | **あり** |
| 情報損失 | — | ゼロ（検証済） | ゼロ（検証済） |
...
- Excel ノイズ: [方眼紙Excel→MD全手法比較](https://zenn.dev/ougotti/articles/houganshi-excel-to-markdown) — LLM が現実解
- Excel 処理: [LLMを悩ませるExcel文書](https://zenn.dev/firstautomation/articles/aed95bce20e900) — PDF化+Vision が複雑構造に有効
- Excel Docling: [セル結合まみれのExcel](https://zenn.dev/labdemy/articles/c4e62ca679b1d1) — HTML出力で rowspan/colspan 保持
